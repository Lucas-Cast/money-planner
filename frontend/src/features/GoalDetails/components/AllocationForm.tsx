...
const allocationSchema = z.object({
  ...
  amount: z
    .number({ message: "Informe o valor alocado." })
    .min(0, "O valor não pode ser negativo."),
  entryPrice: z.number().min(0, "O preço não pode ser negativo.").optional(),
  fxRate: z
    .number()
    .min(0, "A taxa de câmbio não pode ser negativa.")
    .optional(),
  yieldPercent: z
    .number()
...
export function AllocationForm({
  ...
  const {
    ...
    loading: creating,
    error: createError,
  } = usePost<Allocation, AllocationFormValues & { goalId: number }>()
  const {
    patch,
    loading: updating,
    error: updateError,
  } = usePatch<Allocation, AllocationFormValues>()
  const {
    control,
    handleSubmit,
    reset,
  ...
  const numericField = (
    name: "amount" | "entryPrice" | "fxRate" | "yieldPercent",
    label: string,
    errorMessage?: string,
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="mt-7 space-y-5">
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        {numericField(
          "fxRate",
          "Taxa de câmbio",
          errors.fxRate?.message,
          "Opcional para ativos em moeda estrangeira.",
        )}
      </div>
      ...
      {numericField(
        "yieldPercent",
        "Rentabilidade",
        errors.yieldPercent?.message,
        "Opcional. Informe conforme o tipo de ativo.",
      )}
      ...
      {Boolean(error) && (
        ...
        >
          Não foi possível {allocation ? "atualizar" : "criar"} a alocação.
          Tente novamente.
        </p>
      )}
